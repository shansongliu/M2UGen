...
class AnyToAnyInstructionDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        return dict(input_paths=self.input_path_list[i], output_texts=self.instruction_list[i],
                    dataset_types=self.dataset_type_list[i], caption_texts=self.caption_list[i])

    def collate(self, instances):
        input_paths, output_texts, dataset_types, caption_texts = tuple(
            [instance[key] for instance in instances] for key in
            ("input_paths", "output_texts", "dataset_types", "caption_texts"))
        return dict(
